...
def LEsim(b=None, X=None, timefac=1.0, BWpot=0):
	...
	## Output options
	outfile  = "./dat_LE_stream/TRA_"+pot+"_b"+str(b)+"X"+str(int(X))+"n"+str(nstp)+"seed"+str(seed)
	...
	if savedata:
		## Hack to save DB space
		outfile = "C:/AsusWebStorage/MySyncData/TRAFILES/"+os.path.split(outfile)[1]
		save_data(outfile,np.vstack((tarr,x,eta,xi)))

	return np.vstack([tarr,x,eta,xi])#, outfile
